...
class StbDetailController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="javascript:void(0)" data-toggle="tooltip"  data-id="'.$row->id.'" data-original-title="Edit" class="edit btn btn-sm editProduct">Edit</a>';
    ...
    public function cari(Request $request)
    {
        ...
        $pegawai = DB::table('stb_detail')
                    ...
                    ->where('mac_address','like',"%".$cari."%")
                    ->select('stb_detail.stb_id','stb_detail.id','master_vendor_svn.name',DB::raw('DATE_FORMAT(stb_detail.purchase_date, "%d-%m-%Y") as date'), 'stb_detail.product_model','stb_detail.mac_address','stb_detail.serial_number','stb_detail.remarks','stb_detail.status')
                    ->paginate(10);
